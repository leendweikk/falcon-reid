...
def main():
    ...
    print(f"mode {mode} | device {ex.device} | decode {ex.decode} | flip {ex.flip} | "
          f"models {[m['name'] for m in cfg['modes'][mode]['models']]} | dim {ex.dim} | loaded in {t_load:.1f}s")
    ...
    gallery = Gallery(g_emb, g_df.image_id, rr["k1"], rr["k2"], rr["lambda"], rr["topk"],
                      rerank=rr["enabled"] and not args.no_rerank)
    t0 = time.time()
    sub_lines, cands = [], []
    for i, qid in enumerate(q_df.image_id):
        ranked, top, conf = gallery.search(q_emb[i])
    ...
    info = {
        ...
        "seconds": {"load": round(t_load, 2), "embed": round(t_embed, 2), "search": round(t_search, 2),
    ...
    print(f"done -> {out}  | answered {info['answered']}/{len(q_df)} (threshold {threshold}) | "
          f"embed {t_embed:.1f}s ({info['ms_per_image_batched']} ms/image batched), search {t_search:.1f}s")
